...
export default {
	...
	/**
	 * Instead of using nforce's, which requires you to create a connection,
	 * I do this here because I haven't created a connection when I need it
	 */
	getAuthUri(env) {
		const subDomain = env === 'sandbox' ? 'test' : 'login';
		const baseUrl = `https://${subDomain}.salesforce.com/services/oauth2`;
		const redirect = encodeURIComponent(`${baseUrl}/success`);
