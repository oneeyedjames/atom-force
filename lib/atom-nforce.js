...
export default class SfConnection extends EventEmitter {
	...
	retrieveMetadata(rootDir, package) {
		...
		util.readFilePromise(rootDir + '/.checksum')
		.then(jsonData => checksum = JSON.parse(jsonData))
		...
		retrievePromise.then(res => {
			...
			unzipper.Open.buffer(buff).then(res => {
				res.files.forEach(file => {
					...
					if (relpath == 'package.xml') return;

					checksum[relpath] = file.crc32;

					var abspath = rootDir + '/src/' + relpath;
					this._mkdir(abspath);

					file.stream().pipe(fs.createWriteStream(abspath));
				});

				util.writeFilePromise(rootDir + '/.checksum',
