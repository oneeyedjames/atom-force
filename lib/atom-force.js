...
export default class {
	init() {
		atom.commands.add('atom-workspace', 'atom-force:addForceNature', () => {
			console.log('called atom-force:addForceNature');
			this.addForceNature();
		});
	...
	loadProjectData() {
		...
		.catch((err) => {
			...
			console.log('First folder has no Force.com nature.');
			console.error(err);
	...
	addForceNature() {
		console.log('called addForceNature.');
		let env = '';
		this._chooseEnv()
		.then(({ name }) => {
			env = name;
	...
	_initProject() {
		...
		atom.commands.add('atom-workspace', 'atom-force:retrieve', () => {
			this._retrieveMetadata();
		});
		util.addMenuItem('Retrieve All Metadata', 'atom-force:retrieve');
	...
	_deployFiles(filenames) {
		...
		.then(tests => {
			if (projectData.name === 'Primary') {
				...
				const tempConn = new SfConnection(
					projectData.oauth,
					projectData.env
				);
	...
	_retrieveMetadata() {
		...
		.then(projectData => {
			if (projectData.name === 'Primary') {
				this.conn.retrieveMetadata(this.sfProject.projRoot, this.sfProject.package);
			} else {
				const tempConn = new  SfConnection(
					projectData.oauth,
					projectData.env
				);
				tempConn.retrieveMetadata(this.sfProject.projRoot, this.sfProject.package);
			}
		});
	}
	...
	_listenDeploy(conn) {
		...
		conn.on('saveDeploy', oauth => {
			console.log('opening deployment window');
			win.webContents.loadURL(`${oauth.instance_url}/secur/frontdoor.jsp?sid=${oauth.access_token}`
	...
	_getTokenUserAgent(env) {
		...
		return new Promise((resolve, reject) => {
			...
			win.webContents.on('did-finish-load', () => {
				const url_ = win.getURL();
				console.log(`got URL: ${url_}`);
				const urlData = url.parse(url_);
				...
				if (urlData.hash) {
					...
					console.log(`got hash params ${params}`);
